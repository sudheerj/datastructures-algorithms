...
1. Maintain a `diameter` variable initialized to `0`, updated via `nonlocal` in the inner `dfs` function.

2. Define a recursive `dfs(node)` function that returns the **height** of the subtree rooted at `node`.

3. Base case: return `0` if `node` is `None`.

4. Recursively compute `left_height = dfs(node.left)` and `right_height = dfs(node.right)`.

5. Update `diameter = max(diameter, left_height + right_height)` — the widest path through this node spans both subtrees.

6. Return `1 + max(left_height, right_height)` as this node's height to its parent.
