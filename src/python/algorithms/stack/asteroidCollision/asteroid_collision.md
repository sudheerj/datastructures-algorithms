...
2. For each asteroid `a` in the array:
   1. While the stack is not empty AND `stack[-1] > 0` AND `a < 0` (collision):
      - If `stack[-1] < -a`: top is smaller → pop and continue.
      - If `stack[-1] == -a`: equal → pop top, set `a = 0`, break.
      - Else: current is smaller → set `a = 0`, break.
   2. If `a != 0`, append `a` to the stack.
3. Return the stack.
