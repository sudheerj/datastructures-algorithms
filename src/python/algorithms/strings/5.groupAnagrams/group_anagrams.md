...
In this approach, the problem is solved with the combination of **frequency counting** approach using an array and store anagrams in a dictionary. The algorithmic approach can be summarized as follows:
...
3. Use the frequency array (converted to a tuple) as a unique key.
4. Add the string to the group corresponding to this key.
5. Return all grouped anagrams as a list of lists.

### 2. Sorted String as Key
In this approach, the problem is solved with the combination of **sorting string as key** and store anagrams in a dictionary. The algorithmic approach can be summarized as follows:
...
Also, it requires space complexity of `O(m * n)` due to dictionary which stores list of arrays.
