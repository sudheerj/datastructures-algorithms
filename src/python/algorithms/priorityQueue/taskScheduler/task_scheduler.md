...
2. Push all non-zero frequencies as negated values into a min-heap (Python's `heapq`), which simulates a max-heap.
...
4. While the heap is not empty, simulate one CPU cycle of length `n + 1`:
   ...
   b. Process up to `n + 1` tasks from the heap. For each task popped, negate it to get its true frequency, decrement, and if still > 0 add the negated value to `temp`. Increment `time`.
   c. Push all remaining frequencies in `temp` back into the heap.
   d. If the heap is still not empty after the cycle, add the remaining idle slots to `time`.
