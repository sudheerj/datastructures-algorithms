...
**Algorithmic Steps**
This problem is solved by set which verifies whether the element exists or not and extending the consecutive sequence verifying through the next upcoming elements. The algorithmic approach can be summarized as follows: 

1. Create a function (`longest_consecutive_sequence`) to determine the length of longest consecutive sequence of given input array.
   
2. Declare a set (`num_set`) to eliminate the duplicate values from input array. It is mainly used to verify the element existence with `O(1)` time.
   
3. Declare a variable (`longest_streak`) to store the longest consecutive sequence.
...
6. If the left element doesn't exist, it is an indication of new sequence started and initialize the current sequence length (`current_streak`) with a value `1`.
...
8. After completion of current sequence, update the longest sequence length by taking the maximum of itself and current sequence length.
...
10. Return `longest_streak` as the longest consecutive sequence of given array.
...
The `longest_consecutive_sequence` function requires `O(n)` space complexity, where `n` is the number of elements in the input array. This is because a set is created to store the elements of the given input array.
