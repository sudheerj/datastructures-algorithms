...
### 1. **Build the Graph**:
   - Create an adjacency list representation where each variable is a node
   ...
   - Use `defaultdict(list)` for convenient graph construction

### 2. **Process Each Query**:
   For each query `[C, D]`:
   
   **a) Check if both variables exist in the graph**:
   - If either variable is not in the graph, append `-1.0` to results
...
### 4. **Return Results**:
   - Return list of results for all queries
...
- **defaultdict usage**: Simplifies graph construction, automatically creates empty lists
...
- Visited set: `O(V)`
- Result list: `O(Q)` where Q is the number of queries
...
- Use `defaultdict(list)` for easy graph construction
- Tuple unpacking in loops: `for a, b in equations`
- Set for O(1) membership testing in visited tracking
- List comprehension could be used for building results if not using DFS
