...
2. Create a function (`count_connected_components`) to determine the number of connected components in the graph. It accepts `n` as number of nodes and `edges` as node pairs which are connected.

3. Initially each node is its own parent, the parents array is updated with list of node values starting from `0` to `n-1`. These nodes will have ranking 1 because of no separate parent node.
...
9. Thereafter, return `1` indicating that two groups can be merged as one large group.
