...
1. Create a function (`num_islands`) to determine the number of islands in the grid. It accepts `m*n` matrix `grid` as input parameter.
   
2. Calculate the total rows and cols of given grid and store them in `rows` and `cols` variables respectively.

3. The total number of islands is stored in `islands_count` variable. By default, it is initialized to 0.
...
4. Create a dfs function (`dfs`) to traverse each cell of the grid and its neighbours.
   ...
   2. Return immediately either if the cell co-ordinates are not within grid boundaries or already visited.
...
6. Return `islands_count` as number of islands exists in the given grid.

**Time and Space complexity:**
This algorithm has a time complexity of `O(rows * cols)`, where `rows` is the number of rows in the island and `cols` is the number of columns in the island. This is because each cell is visited exactly once during the dfs traversal using a recursion stack.

The `num_islands` function requires `O(rows * cols)` space complexity, where `rows` is the number of rows in the island and `cols` is the number of columns in the island. This is because when the entire grid is filled with lands (`'1'`), the call stack will grow to the number of cells in the grid.
