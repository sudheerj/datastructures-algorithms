...
1. Create a `UnionFind` class with:
   - `parent` initialized to `parent[i] = i` for all `i` using `list(range(n))`.
   - `rank` initialized to `[0] * n`.
   ...
   - `get_count()` — returns current number of groups.

2. Create the main function `find_time(logs, n)`.

3. Sort `logs` in ascending order by timestamp using `logs.sort(key=lambda x: x[0])`.

4. For each log `[time, p1, p2]`:
   1. Call `union(p1, p2)`. If they were already connected, skip.
   2. After union, check `get_count() == 1`. If so, all are connected — return `time`.
...
The space complexity is `O(n)` for the `parent` and `rank` lists.
