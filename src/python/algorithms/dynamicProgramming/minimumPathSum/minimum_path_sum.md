...
### Approach 1 — Top-down Memoization `min_path_sum1`
1. Start DFS from `(0, 0)`.
2. If out of bounds, return `inf`.
3. If at `(rows-1, cols-1)`, return `grid[r][c]`.
4. If `(r, c)` in memo, return cached result.
5. `memo[(r,c)] = grid[r][c] + min(dfs(r+1,c), dfs(r,c+1))`.

### Approach 2 — Bottom-up DP `min_path_sum2`
1. Create `dp[rows+1][cols+1]` filled with `inf`.
2. Set `dp[rows-1][cols] = 0`.
3. For `r` from `rows-1` to `0`, `c` from `cols-1` to `0`:
   `dp[r][c] = grid[r][c] + min(dp[r+1][c], dp[r][c+1])`.
4. Return `dp[0][0]`.

### Approach 3 — Space-Optimized DP `min_path_sum3`
1. `dp = [inf] * (cols+1)`, set `dp[cols-1] = 0`.
