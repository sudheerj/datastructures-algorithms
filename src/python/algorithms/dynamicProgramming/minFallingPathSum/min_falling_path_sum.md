...
### Approach 1 — Top-down Memoization `min_falling_path_sum1`
1. Use a `memo` dict keyed by `(r, c)`.
2. Try every starting column `c` in row 0, return the minimum.
3. `dfs(r, c)`:
   - `r == n` → return `0`.
   - `c < 0 or c >= n` → return `inf`.
   - `memo[(r,c)] = matrix[r][c] + min(dfs(r+1,c-1), dfs(r+1,c), dfs(r+1,c+1))`.

### Approach 2 — Bottom-up In-place `min_falling_path_sum2`
1. Copy the matrix to avoid mutating input.
2. For each row `r` from `1` to `n-1`, for each `c`:
   - `left  = m[r-1][c-1]` if `c > 0`, else `inf`
   - `mid   = m[r-1][c]`
   - `right = m[r-1][c+1]` if `c < n-1`, else `inf`
