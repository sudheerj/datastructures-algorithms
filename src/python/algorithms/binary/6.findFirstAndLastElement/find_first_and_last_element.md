...
1. Define a helper function `find_bound(nums, target, find_first)` that performs a modified binary search.
...
5. Call `find_bound` twice — once with `find_first = True` and once with `find_first = False` — to get the starting and ending positions.

6. Return the result as `[first_index, last_index]`.
