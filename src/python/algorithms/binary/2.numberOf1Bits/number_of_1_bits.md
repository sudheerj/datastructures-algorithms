...
**Algorithmic Steps:**
This problem is solved with the help of **Brian Kernighan's** technique. The main intuition behind this algorithm is that when you subtract 1 from an integer, all the bits following the rightmost set bit are inverted (i.e, turning 1 to 0 and 0 to 1). The algorithmic approach can be summarized as follows: 
...
3. Iterate the number until it becomes zero. 
   
4. Update the number by ANDing the number itself with number which is subtracted by 1. It helps to flip the right most bit everytime.
