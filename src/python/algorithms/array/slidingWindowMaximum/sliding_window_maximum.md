...
2. For each element at index `right`:
   ...
   - Remove from the **front** if the index has fallen outside the window (`left > deque[0]`).
   - Once the window is full (`right + 1 >= k`), record `nums[deque[0]]` as the max for this window, then advance `left`.
