...
Uses `math.ceil` for bucket size and `float('inf')` / `float('-inf')` as empty markers.
...
2. Find global `min_val` and `max_val`.
...
6. Scan buckets: `max_gap = max(max_gap, current_bucket_min - previous_bucket_max)`.
7. Return `max_gap`.
...
Scan:
  bucket 0: 1 - 1(global min) = 0,  prev_max = 3
  bucket 1: 6 - 3 = 3,              prev_max = 6
  bucket 2: 9 - 6 = 3,              prev_max = 9
...
```
min=1, max=10, n=4
bucket_size = ceil(9/3) = 3
bucket_count = 9//3 + 1 = 4
...
Scan:
  bucket 0: 1 - 1 = 0,   prev_max = 3
  bucket 1: 6 - 3 = 3,   prev_max = 6
  bucket 2: skip (empty)
  bucket 3: 10 - 6 = 4,  prev_max = 10
