...
**Algorithmic Steps**
This problem is solved using a **sorted array of fixed size 5** per student (simulating a min-heap).

1. Iterate through all `[id, score]` pairs.
2. For each student, maintain a sorted ascending array capped at 5 elements:
   - Binary-search the insertion position and splice the score in.
   - If the array exceeds length 5, `shift()` to evict the minimum (leftmost) element.
3. After all items are processed, for each student in sorted id order:
   - Sum the array values.
   - Compute `Math.floor(sum / 5)`.
4. Return `[[id, avg], ...]`.
