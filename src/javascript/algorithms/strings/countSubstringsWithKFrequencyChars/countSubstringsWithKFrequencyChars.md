...
5. While the frequency of the current `rightChar` is ≥ `k`, all substrings that start at any index from `left` to `right` and end at `right` or beyond contain `rightChar` at least `k` times. Add `n - right` to `result`.
