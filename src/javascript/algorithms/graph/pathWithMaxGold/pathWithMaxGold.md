...
1. Create the main function `pathWithMaxGold(grid)` with a closure variable `maxGold = 0`.

2. Define an inner `dfs(r, c, currentGold)` function.

3. In the `dfs` function:
   1. **Base case:** if out of bounds or `grid[r][c] === 0`, return immediately.
   2. Read `gold = grid[r][c]`, add to `currentGold`.
   3. Update `maxGold = Math.max(maxGold, currentGold)`.
