...
2. Load all words into a `wordSet` (Set). Return empty list if `endWord` is not in `wordSet`.
...
4. BFS level-by-level:
   ...
   3. If the new word exists in `wordSet` and has NOT been seen in a previous level (`!visited.has(nextWord)`), add it to the next queue and `levelVisited`.
   4. Always record the parent: `parents.get(nextWord).push(word)` (handles multiple parents for same shortest path).
   5. If `nextWord === endWord`, set `found = true`.
...
5. DFS backtracking from `endWord` to `beginWord` using `parents` map:
   ...
   3. When `beginWord` is reached, reverse the path and push to result.
