...
2. **Define DFS helper function**:
   - The DFS function takes row and column coordinates
...
3. **Start DFS from the starting position**:
   - Call the DFS helper function with the starting position (sr, sc)
...
- **Visited tracking**: By changing the color immediately when visiting a pixel, we implicitly mark it as visited (no separate visited set needed)
