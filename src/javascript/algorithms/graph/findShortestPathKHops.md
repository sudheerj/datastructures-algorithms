...
### 1. **Initialize Data Structures**:
   ...
   - Create a min-heap (priority queue) that stores objects `{cost, node, hops}`, ordered by cost
...
### 3. **Explore Neighbors**:
   ...
   - Calculate `newCost = currentCost + edge.weight`
   - Calculate `newHops = currentHops + 1`
   - If `newCost < best[neighbor][newHops]`:
     - Update `best[neighbor][newHops] = newCost`
     - Add new state `{cost: newCost, node: neighbor, hops: newHops}` to heap
...
- **Min-heap ordering**: Custom MinHeap class ensures we find the cheapest path first
...
Queue-based approach using arrays tracking (node, cost, stops).
...
- Custom `MinHeap` class implementation for priority queue (JavaScript doesn't have built-in PQ)
- Use `Array.from()` with callback for 2D array initialization
- Class-based approach for Edge and State objects
- Destructuring in loop: `for (const edge of graph[node])`
- `Infinity` represents unreachable/undefined cost
