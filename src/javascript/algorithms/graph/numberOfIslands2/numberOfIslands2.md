...
2. Create the main function `numberOfIslands2(m, n, positions)`.

3. Convert 2D coordinates `(row, col)` to a 1D index using `row * n + col`.

4. For each position `(row, col)` in `positions`:
   1. Compute the 1D `index`. If `isLand(index)` is already true, push current count and `continue`.
   ...
   4. Push `islandsCount` to the result array.
