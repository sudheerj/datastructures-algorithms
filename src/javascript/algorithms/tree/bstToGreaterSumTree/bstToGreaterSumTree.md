...
1. Maintain a running `sum` variable initialized to `0` (closed over by the inner `dfs` function).
...
3. At each node, add the node's value to `sum`, then set the node's value to `sum`.
