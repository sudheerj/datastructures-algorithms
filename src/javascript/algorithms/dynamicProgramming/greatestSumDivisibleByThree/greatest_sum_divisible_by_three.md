...
1. **Initialize a DP array**: Create an array `dp` of size 3 where:
...
2. **Process each number**: For each number in the input array:
   - Create a snapshot of the current `dp` state using spread operator (to avoid overwriting during iteration)
...
- **State transition**: When we add a number to an existing sum, the new remainder is `(oldSum + num) % 3`.
...
- We only use a fixed-size array of 3 elements regardless of input size
- The temporary snapshot array is also of size 3
...
## Running the tests:
```bash
node greatest_sum_divisible_by_three.js
```
