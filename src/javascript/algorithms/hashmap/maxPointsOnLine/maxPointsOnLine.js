...
const testCases = [
    { points: [[1,1],[2,2],[3,3]],                   expected: 3, label: "all collinear" },
    { points: [[1,1],[3,2],[5,3],[4,1],[2,3],[1,4]], expected: 4, label: "LeetCode example 2" },
    { points: [[0,0]],                               expected: 1, label: "single point" },
    { points: [[0,0],[1,0],[2,0]],                   expected: 3, label: "horizontal line" },
    { points: [[0,0],[0,1],[0,2]],                   expected: 3, label: "vertical line" },
    { points: [[0,0],[0,0],[1,1]],                   expected: 3, label: "duplicate points" },
];

testCases.forEach(({ points, expected, label }) => {
    const result = maxPointsOnLine(points);
    const status = result === expected ? "PASS" : "FAIL";
    console.log(`  [${status}] ${label}: ${JSON.stringify(points)} → ${result} (expected ${expected})`);
});
