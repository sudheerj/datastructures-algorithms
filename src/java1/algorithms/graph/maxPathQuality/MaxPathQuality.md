...
3. Initialize a `visitedCount[]` array of size `n` (tracks how many times each node is on the current path).

4. Call `dfs(node=0, time=0, score=0, visitedCount, maxTime)`.

5. In the `dfs` function:
   1. If `visitedCount[node] == 0` (first visit), add `values[node]` to `score`.
   2. Increment `visitedCount[node]`.
   3. If `node == 0`, update `maxScore = max(maxScore, score)` (valid return to origin).
   4. For each neighbor `(nei, cost)`: if `time + cost <= maxTime`, recursively call `dfs(nei, time+cost, score, ...)`.
   5. After exploring, decrement `visitedCount[node]` (backtrack).
