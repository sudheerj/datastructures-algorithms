...
### 2. **Process Each Query**:
   ...
   **b) Handle special case**:
   - If `C == D` and the variable exists, return `1.0`
...
### 3. **DFS Helper Function**:
   ```
   dfs(current, target, product, visited):
       if current == target:
           return product
       ...
       for each neighbor of current:
           if neighbor not visited:
               result = dfs(neighbor, target, product * edge_weight, visited)
               if result != -1.0:
                   return result
    ...
    while (!queue.isEmpty()) {
        ...
        for (Edge edge : graph.get(curr.node)) {
            if (!visited.contains(edge.node)) {
                double newProduct = curr.product * edge.weight;
                if (edge.node.equals(dst)) {
                    return newProduct;
                }
                queue.offer(new Pair(edge.node, newProduct));
...
## Running the tests:
```bash
javac EvaluateDivision.java
java EvaluateDivision
```
