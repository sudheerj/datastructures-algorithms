...
### 1. Queue Simulation (winnerOfCircularGame1) — TC: O(n * k), SC: O(n)

1. Add friends `1` to `n` into a queue in order.

2. While the queue has more than one element:
...
### 2. Iterative Josephus (winnerOfCircularGame2) — TC: O(n), SC: O(1)
...
### 3. Recursive Josephus (winnerOfCircularGame3) — TC: O(n), SC: O(n)

1. Define a helper `josephus(n, k)` that returns the 0-based index of the winner for `n` people.

2. Base case: `josephus(1, k) = 0` (only one person, index 0).

3. Recursive case: `josephus(n, k) = (josephus(n-1, k) + k) % n`
...
- Recursive Josephus: Time `O(n)`, Space `O(n)` (recursive call stack)
