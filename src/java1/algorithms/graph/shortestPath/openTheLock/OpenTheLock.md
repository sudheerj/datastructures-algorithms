...
2. Build a `dead` set from the `deadends` array for O(1) lookup.

3. If `dead` contains `"0000"`, return immediately (cannot start).

4. Initialize a BFS queue with `"0000"`, a `visited` set containing `"0000"`, and `steps = 0`.

5. While the queue is not empty:
   1. Record the current level `size = queue.size()`, then `steps++`.
   2. For each node in the level, dequeue `current`. If `current.equals(target)`, return `steps`.
   3. Convert `current` to a char array. For each of the 4 wheel positions `j`:
      - Save `original = arr[j]`.
      - Try both directions (`+1` and `-1`): compute `next = ((digit + move + 10) % 10)`.
      - If `next` is not in `dead` and not yet visited, add to queue and mark visited.
      - Restore `arr[j] = original` before moving to the next wheel.
