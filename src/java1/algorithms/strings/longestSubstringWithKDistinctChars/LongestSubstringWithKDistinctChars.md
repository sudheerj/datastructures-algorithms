...
**Algorithmic Steps**
This problem is solved with the help of the **sliding window** approach using a HashMap. The algorithmic approach can be summarized as follows:

1. Add preliminary corner case conditions: if the string is null/empty or `k` is 0, return 0.

2. Initialize a HashMap (`charFrequencyMap`) to store each character in the current window and its frequency.
...
6. While the map contains more than `k` distinct characters, shrink the window from the left: decrement the frequency of `str[left]`, remove it from the map if its frequency reaches 0, then increment `left`.
...
Also, it requires a space complexity of `O(k)` for the HashMap, which stores at most `k + 1` distinct characters at any time.
