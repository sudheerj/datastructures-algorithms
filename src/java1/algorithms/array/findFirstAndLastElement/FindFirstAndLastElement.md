...
1. Define a helper method `findBound(int[] nums, int target, boolean findFirst)` that performs a modified binary search.
...
5. Call `findBound` twice via `searchRange` — once with `findFirst = true` and once with `findFirst = false` — to get the starting and ending positions.
