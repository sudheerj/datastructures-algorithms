...
3. Compute `bucketSize = ceil((max - min) / (n - 1))`.
4. Compute `bucketCount = (max - min) / bucketSize + 1`.
5. Place each element into its bucket (store only bucket `min` and `max`).
6. Scan buckets: `maxGap = max(maxGap, currentBucketMin - previousBucketMax)`.
7. Return `maxGap`.
...
Scan:
  bucket 0: gap = 1 - 1(global min) = 0,  prevMax = 3
  bucket 1: gap = 6 - 3 = 3,              prevMax = 6
  bucket 2: gap = 9 - 6 = 3,              prevMax = 9
...
Scan:
  bucket 0: 1 - 1 = 0,  prevMax = 3
  bucket 1: 6 - 3 = 3,  prevMax = 6
  bucket 2: skip (empty)
  bucket 3: 10 - 6 = 4, prevMax = 10
