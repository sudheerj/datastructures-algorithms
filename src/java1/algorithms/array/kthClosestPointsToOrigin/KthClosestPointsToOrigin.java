...
public class KthClosestPointsToOrigin {
    ...
    // Approach 4: QuickSelect TC: O(n) avg, O(n²) worst SC: O(log n) recursion stack
    ...
    private static void quickSelect(int[][] points, int left, int right, int k) {
        ...
        if (pivotIndex < k) quickSelect(points, pivotIndex + 1, right, k);
    ...
    private static boolean equals(int[][] a, int[][] b) {
        ...
        for (int i = 0; i < ac.length; i++) {
            if (!Arrays.equals(ac[i], bc[i])) return false;
        }
        return true;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int approach = 0; approach < 4; approach++) {
            ...
            for (int i = 0; i < ks.length; i++) {
                ...
                System.out.println("[" + (pass ? "PASS" : "FAIL") + "] result=" + fmt(result) + " expected=" + fmt(expecteds[i]));
            }
        }
    }
}
