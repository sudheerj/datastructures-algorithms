...
public class Triangle {
    ...
    private static int dfs(List<List<Integer>> triangle, int r, int c, int[][] memo) {
        ...
        return memo[r][c] = triangle.get(r).get(c)
                + Math.min(dfs(triangle, r + 1, c, memo), dfs(triangle, r + 1, c + 1, memo));
    }
    ...
    private static int triangle3(List<List<Integer>> triangle) {
        ...
        for (int r = n - 2; r >= 0; r--) {
            for (int c = 0; c < triangle.get(r).size(); c++) {
                triangle.get(r).set(c,
                        triangle.get(r).get(c) + Math.min(triangle.get(r + 1).get(c), triangle.get(r + 1).get(c + 1)));
            }
        }
